...
namespace CuoiKy
{
    internal class Program
    {
        ...
        public class BinarySearchTree
        {
            ...
            public Node Find(Account account)
            { return this.Find(account, this.root); }
            private Node Find(Account account, Node parent)
            {
                if (parent != null)
                {
                    if (account.id == parent.Data.id) return parent;
                    if (account.id < parent.Data.id) return Find(account, parent.LeftNode);
                    else
                        return Find(account, parent.RightNode);
                }
                return null;
            }
        ...
        static void Main(string[] args)
        {
            ...
            Account newaccount3 = new Account("", "", "", "", 3223, 20);

            binarytree.Exist(newaccount3);
            binarytree.Exist(newaccount1);
